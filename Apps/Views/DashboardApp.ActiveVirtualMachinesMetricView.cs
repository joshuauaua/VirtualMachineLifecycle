/*
The number of virtual machines with an active status.
SELECT COUNT(*) FROM vms WHERE VmStatus.DescriptionText = 'Active'
*/
namespace VirtualMachineLifecycle.Apps.Views;

public class ActiveVirtualMachinesMetricView(DateTime fromDate, DateTime toDate) : ViewBase
{
    public override object Build()
    {
        var factory = UseService<VirtualMachineLifecycleContextFactory>();

        async Task<MetricRecord> CalculateActiveVirtualMachines()
        {
        ...
        return new MetricView(
            ...
            Icons.Activity,
            CalculateActiveVirtualMachines
        );
    }
}