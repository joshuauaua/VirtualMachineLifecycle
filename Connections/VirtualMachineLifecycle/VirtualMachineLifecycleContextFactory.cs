...
public sealed class VirtualMachineLifecycleContextFactory : IDbContextFactory<VirtualMachineLifecycleContext>
{
    ...
    private readonly string _uniqueId = "3c1d0937";
    private readonly ILogger? _logger;

    public VirtualMachineLifecycleContextFactory(
        ServerArgs args,
        IVolume? volume = null,
        ILogger? logger = null
    )
    {
        ...
        var volume1 = volume ?? new FolderVolume(
            System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Ivy-Data", "VirtualMachineLifecycle"));
        _absolutePath = volume1.GetAbsolutePath(_uniqueId + "." + _relativePath);
        _logger = logger;
    }

    public VirtualMachineLifecycleContext CreateDbContext()
    {
        EnsureDatabasePresentOnce();

        var optionsBuilder = new DbContextOptionsBuilder<VirtualMachineLifecycleContext>()
        ...
        return new VirtualMachineLifecycleContext(optionsBuilder.Options);
    }
