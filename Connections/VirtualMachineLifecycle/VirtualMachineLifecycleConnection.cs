...
public class VirtualMachineLifecycleConnection : IConnection, IHaveSecrets
{
    public string GetContext(string connectionPath)
    {
        var connectionFile = nameof(VirtualMachineLifecycleConnection) + ".cs";
        var contextFactoryFile = nameof(VirtualMachineLifecycleContextFactory) + ".cs";
    ...
    public string GetName() => nameof(VirtualMachineLifecycle);

    public string GetNamespace() => typeof(VirtualMachineLifecycleConnection).Namespace;

    public string GetConnectionType() => "EntityFramework.Sqlite";

    public ConnectionEntity[] GetEntities()
    {
        return typeof(VirtualMachineLifecycleContext)
            .GetProperties()
